...
$\tau$.  Per determinare le altre equazioni consideriamo l'equazione della
geodetica
\begin{equation}
  \toder[2]{x^{\mu}}{\tau}+\tensor{\Gamma}{^{\mu}_{\nu\rho}}
  \toder{x^{\nu}}{\tau} \toder{x^{\rho}}{\tau} = 0
\end{equation}
in cui gli elementi della connessione affine sono calcolati a partire dalla
metrica di Schwarzshild.  Ponendo in questa equazione $\mu=2$, $\mu=3$ e
$\mu=0$, si ottengono tre equazioni per le coordinate $\theta$, $\phi$ e $t$,
rispettivamente
\begin{subequations}
  \begin{gather}
...
Non abbiamo scritto l'equazione di Eulero-Lagrange per la coordinata $r$ ($\mu
=1$) poiché utilizzeremo la~\eqref{eq:mercurio-r}.

Si osservi che le equazioni precedenti possono essere ottenute risolvendo il
problema variazionale per la Lagrangiana $\mathcal{L} = (1/2) g_{\mu \nu}
\dot{x}^{\mu} \dot{x}^{\nu}$, che per la metrica di Schwarzshild è
\begin{equation}
  2 \mathcal{L} = \bigg(1 - \frac{r_{\textup{S}}}{r}\bigg)\dot{t}^{2} -
  \bigg(1 - \frac{r_{\textup{S}}}{r}\bigg)^{-1}\dot{r}^{2} -
  r^{2}(\dot{\theta}^{2} + \sin^{2}\theta \dot{\phi}^{2}).
\label{straumann321}
\end{equation}
Ovviamente lungo la geodetica $2 \mathcal{L} = 1$.  Per il principio di minima
azione
\begin{equation}
...
\end{equation}
da cui si ottengono le tre equazioni precedenti per le coordinate $\theta$,
$\phi$ e $t$.
...
Prima di procedere, osserviamo che dall'equazione~\eqref{straumann321} si vede
che le variabili $\phi$ e $t$ sono cicliche, per cui il momento angolare e
l'energia (per unità di massa) si conservano:
\begin{subequations}
  \begin{align}
    \parder{\mathcal{L}}{\dot{\phi}} &= r^2 \dot{\phi} = \text{cost} = L, \\
    \parder{\mathcal{L}}{\dot{t}} &= \biggl(1 - \frac{r_{\textup{S}}}{r}\biggr)
    \dot{t} = \text{cost} = E.
  \end{align}
\end{subequations}
...
un moto circolare. In questo caso non c'è un perielio. L'altra
...
a riposo.  Tuttavia dobbiamo fare attenzione al fatto che, proprio in virtù del
fatto che $\dd\tau^{2} = 0$, non possiamo parametrizzare la linea d'universo di
un fotone, o di una qualsiasi particella con massa nulla, usando il tempo
...
\end{equation}
abbia senso (non è necessario precisare il senso fisico di questo parametro che
non comparirà nelle equazioni finali).  A partire da questa equazione si possono
ricavare nuovamente, sempre con l'assunzione $\theta = \pi/2$, le
relazioni~\eqref{eq:mercurio-phi2} e \eqref{eq:mercurio-t2}.
La~\eqref{eq:mercurio-r2} va modificata in questo modo tenendo conto del fatto
che $\dd\tau^{2} = 0$
\begin{equation}
...
teoriche della relatività generale.\footnote{Questo problema è trattato da
  \textcite[1103-1109]{misner:gravitation};
  \textcite[201-207]{weinberg:gravitation}.}  In un spazio-tempo piatto, il
tempo $t(r,R_{\odot})$ necessario per un fotone per giungere tangente sulla
superficie del Sole da un punto a distanza $r$ dal centro della stella è dato
semplicemente dal rapporto fra la distanza euclidea fra questi due punti (vedi
figura~\ref{fig:ritardo-radar}) e la velocità ($c=1$) dei fotoni nel vuoto
\begin{equation}
  \tau(r, R_{\odot}) = \frac{\sqrt{r^{2} - R_{\odot}^{2}}}{c}.
\end{equation}
La correzione relativistica per $t(r, R_{\odot})$, che tiene conto della
curvatura dello spazio tempo prodotta dal campo gravitazionale del Sole, è
\begin{equation}
  \begin{split}
    t(r,R_{\odot}) &\approx t(r,R_{\odot}) + \var t(r,R_{\odot}) \\
    &= \frac{\sqrt{r^{2} - R_{\odot}^{2}}}{c} + \frac{2GM_{\odot}}{c^{3}}
    \ln \left[ \frac{r + \sqrt{r^{2} - R_{\odot}^{2}}}{R_{\odot}} \right] +
...
\begin{equation}
  \begin{split}
    ...
    &= 2(t(r_{1},R_{\odot}) + \var t(r_{1},R_{\odot}) +
    t(r_{2},R_{\odot}) + \var t(r_{2},R_{\odot})).
  \end{split}
\end{equation}
Considerando il sistema Terra-Mercurio, il ritardo $\var t = 2[\var
t(r_{1},R_{\odot}) + \var t(r_{2},R_{\odot}) ]$ rispetto alla previsione
$2[t(r_{1},R_{\odot}) + t(r_{2},R_{\odot})]$ in uno spazio-tempo piatto vale
\SI{240}{\micro\second} (corrispondente ad una distanza di \SI{72}{\kilo\metre})
su un tempo di volo totale $t_{\textup{tot}}$ di \SI{20}{\minute}.

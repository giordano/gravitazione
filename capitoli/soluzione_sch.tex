...
temporale.\footnote{Ricordiamo che l'ipotesi di staticità di un campo è più
  forte della sola stazionarietà, che richiede solo che il campo sia
  indipendente dalla coordinata temporale.}  Queste condizioni impongono che il
campo gravitazionale dipenda dalle coordinate spaziali solo attraverso gli
invarianti per rotazione che possono essere costruiti a partire da $\bm{x}$ e
$\dd\bm{x}$, cioè $\bm{x} \cdot \dd\bm{x}$, $\bm{x} \cdot \bm{x}$, $\dd\bm{x}
\cdot \dd\bm{x}$.  La forma più generale della metrica con queste proprietà è
\begin{equation}

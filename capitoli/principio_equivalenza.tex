...
Usando ancora l'analogia con il campo magnetico e dipolo magnetico,
...
\begin{equation}
  \bm{\tau} = \frac{ d\bm{S}} {dt},
\end{equation}
si trova
\begin{equation}
  \frac{d\bm{S}} {dt} = - \bm{b}/2 \times \bm{S} = \dot{\bm{\Omega}} \times
  \bm{S},
\end{equation}
...
$\norm{\bm{r}} \approx R_{\oplus}$.  Poiché inoltre $\bm{L}_{\oplus} = 5.86
\times 10^{40}$ g cm$^2$ s$^{-1}$, la velocità angolare di precessione
precedente corrisponde a uno spostamento angolare in un anno
$\alpha_{\textup{LT}} = \dot{\bm{\Omega}}_{\textup{LT}} \times 1 yr \approx
-\SI{0.055}{\arcsecond}$.
...
Lense-Tirring.  L'esperimento usa 3 giroscopi raffreddati alla temperatura
...
giroscopio: è il momento angolare orbitale $\bm L$ del satellite (sostituisce lo
spin del giroscopio dell'esperimento GP-B) che precede con velocità
...
terrestre).  L'angolo di Lense-Tirring corrisponde ad un avanzamento della
posizione dei nodi $\var l_{\textup{LT}} = \alpha_{\textup{LT}} \cdot R_{\oplus}
\approx -\SI{2}{\metre/anno}$.  L'analisi dei dati conferma l'effetto
Lense-Tirring con una precisione del 10\%.

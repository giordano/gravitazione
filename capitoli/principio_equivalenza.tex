...
di Lorentz~\eqref{eq:forza-lorentz}.  Inoltre usando la definizione di
$f_{\alpha\beta}$ si verifica che risulta
\begin{equation}
...
\end{equation}
Inoltre il momento torcente agente su un giroscopio di momento angolare $\bm{S}$
è
\begin{equation}
...
visto che $\nabla \times \bm{b} = 16 \pi G \bm{S}$ (qui $\bm{S} = \rho \bm{v}$)
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
Abbiamo quindi trovato che $\bm{b}$ è legato al momento angolare del corpo che
genera il campo gravitazionale, coerentemente con la previsione che il campo
gravitomagnetico è associato a masse in rotazione.  Il momento torcente agente
su un giroscopio con momento angolare $\bm{S}$ (è un vettore distinto dal
vettore $\bm{S} = \rho \bm{v}$) e ``momento di dipolo''
$\bm{m}_{\textup{g}}' = \bm{S}/2$ è
\begin{equation}
...
\begin{equation}
  ...
  -\frac{G}{c^{2}}\frac{3\versor{r}(\versor{r} \cdot \bm{L}) -
    \bm{L}}{\norm{\bm{r}}^{3}}.
\end{equation}
Se il campo gravitomagnetico $\bm{b}$ è non nullo si ha quindi una precessione
di un giroscopio attorno a $\bm{b}$.  Notiamo che $\bm{b}$ è definito a partire
dalle componenti $h_{0k}$ della perturbazione del tensore metrico.  Il tensore
metrico associato a un campo gravitazionale generato da un corpo sferico in
rotazione ha termini non diagonali diversi da zero già nell'approssimazione di
campo debole (vedi~\textcite[192]{ohanian:gravitazione}).  Dunque in questo caso
si ha $h_{0k} \neq 0 \implies \bm{b} \neq 0$ e un giroscopio in orbita attorno a
una massa in rotazione sarà soggetto a una precessione, chiamata
...
$100$ più importante rispetto a quella di Lense-Thirring, quindi quest'ultima è
...
$\phi(r) = -GM/r$, quindi l'equazione precedente si può scrivere come
\begin{equation}
  \frac{\var\tau}{\tau} = \frac{\dd\tau_{2} - \dd\tau_{1}}{\dd\tau_{1}} =
...
periodico misurata da chi riceva il segnale, in un punto con potenziale meno
...
ha il minimo dell'effetto Doppler relativistico.

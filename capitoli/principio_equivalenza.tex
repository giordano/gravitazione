...
gravitazionale $\bm{g}$.  In un sistema di riferimento $\xi^{\alpha}$ in cui
questa si muove di moto rettilineo uniforme nello spazio tempo l'equazione del
moto è
\begin{equation}
...
\begin{equation}
  ...
  u^{\nu} u^{\nu} = 0.
\end{equation}
La~\eqref{eq:geodesica} è chiama \emph{equazione della geodesica}.
...
\section{Relazione fra $g_{\mu\nu}$ e $\tensor{\Gamma}{^{\lambda}_{\mu\nu}}$}
...
condizione che descrive il limite newtoniano.  Notiamo che per $\sigma=0$
...
% TODO: fare la tabellina con il valore del raggio di Schwarzschild per alcuni
% corpi.

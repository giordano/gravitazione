...
% TODO: controllare effettivamente il segno dell'azione, sul Weinberg sta con il
% segno -.
\begin{equation}
...
Concludiamo dicendo che si può dimostrare il \index{teorema!di Birkhoff}
...
\subsection{\completare{Precessione del perielio}}
...
chiusa ma piuttosto una rosetta, si ha quindi una precessione del perielio.  Per
esempio, dalle osservazioni astronomiche si è scoperto che il perielio di
Mercurio si sposta di circa \ang{;;574} ogni cento anni.  Utilizzano la
meccanica newtoniana e tenendo conto della presenza di tutti i pianeti
conosciuti si prevede una precessione del perielio di Mercurio pari a circa
\ang{;;532}.  Per risolvere questa incongruenza sono state proposte diverse
ipotesi, fra cui la presenza di un corpo perturbante, come un nuovo pianeta o un
satellite di Mercurio.  Tuttavia questi corpi non sono mai stati trovati e
faremo vedere che la relatività generale è in grado di risolvere questo
problema.

% TODO: capire per bene i passaggi dell'Adler
Per determinare l'orbita del pianeta dobbiamo risolvere il problema variazionale
\begin{equation}
  \delta \int \dd \tau = 0
\end{equation}
in cui $\delta\tau$ è la metrica di Schwarzschild.  Questo problema può essere
...
\end{equation}
Il punto indica la derivazione rispetto al tempo proprio $\tau$.  Da qui si
ottengono tre equazioni di Eulero-Lagrange per le coordinate, rispettivamente,
$\theta$, $\phi$ e $t$
\begin{subequations}
  \begin{gather}
  ...
  \end{gather}
\end{subequations}
Non abbiamo usato l'equazione di Eulero-Lagrange per la coordinata $r$ poiché è
più semplice dividere l'intervallo di tempo proprio di
Schwarzschild~\eqref{eq:metrica-schwarzschild} per $\dd\tau^{2}$ ottenendo
questa equazione differenziale in $r$
\begin{equation}
  \label{eq:mercurio-r}
  1 = \bigg(1 - \frac{r_{\textup{S}}}{r}\bigg)\dot{t}^{2} - \bigg(1 -

...
rotatore rigido, sistemi di stelle binarie, urto frontale di due oggetti, buchi
neri supermassivi al centro delle galassie, collasso gravitazionale di un corpo
non sfericamente simmetrico.  In questo paragrafo ne considereremo alcuni in
particolare.
...
l'intero sistema di dimensione $r$ alla velocità $v$.  Il momento di inerzia è
dato da $\int \rho x_{i} x_{j} \dd V$, possiamo approssimare allora ogni
componente del tensore momento di quadrupolo di massa con
\begin{equation}
  Q_{ij} \approx 3 \int \rho x_{i} x_{j} \dd V = 3 M r^{2}
\end{equation}
e si può stimare la sua derivata temporale terza dividendo $Q_{ij}$ per $T^{3}$
\begin{equation}
  \dddot{Q}_{ij} \approx 3 \frac{Mr^{2}}{T^{3}} = 3 \frac{Mv^{3}}{r}.
\end{equation}
Poniamo $L_{0} = c^{5}/G = \SI{3.6e59}{\erg\per\second}$, allora
\begin{equation}
  I = \frac{G}{45c^{5}} \dddot{Q}_{ij} \dddot{Q}^{ij} \approx \frac{G}{45c^{5}}
  \frac{G^{2}}{c^{10}} \bigg(3 \frac{M v^{3}}{r}\bigg)^{2} \frac{c^{10}}{G} \sim
...
energia cinetica e $U = -GM/r$ energia potenziale gravitazionale, risulta
\begin{equation}
...
$\mu = m_{1}m_{2}/(m_{1} + m_{2})$, si trova (\textcite[461]{landau:campi},
\textcite[986]{misner:gravitation}, \textcite[476]{shapiro:black-holes}) che
l'energia $I$ emessa nell'unità di tempo sotto forma di onde gravitazionali è
data da
\begin{equation}
...
Collasso gravitazionale di un corpo non sfericamente simmetrico
\begin{equation}
  e = \frac{a-b}{(a+b)/2}
\end{equation}
$M = \SI{1}{\solarmass}$

...
La connessione affine compare anche in uno spazio piatto se la metrica è
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
Consideriamo ora il differenziale del vettore $A^i$ nel sistema $x^i$:
\begin{subequations}
  \begin{align}
...
Nelle precedenti abbiamo introdotto la connessione affine che abbiamo definito
essere:
\begin{equation}
...
Questo algoritmo ci permette, per esempio, di ricavare in maniera immediata
...
\begin{equation}
  \toder{V^{\alpha}}{\tau} = 0,
\end{equation}
in cui $V^{\alpha}$ è la \index{quadrivelocità}quadrivelocità della particella.  In
...
Valutiamo ora le componenti della connessione affine.  Si ha:
\begin{equation}
  \tensor{\Gamma}{^{0}_{01}} =
...
Valutiamo ora l'equazione~\eqref{eqspin} nel punto di coordinate $x=r_0$, $y=0$,
$z=0$
\begin{subequations}
  \begin{align}
...
Queste equazioni sono valide nel punto di coordinate $x=r_0$, $y=0$, $z=0$.
...
\begin{equation}
  ...
  \bm{\Omega}_{\textup{De Sitter}} \times \bm{S}.
\end{equation}
